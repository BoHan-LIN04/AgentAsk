...
class Graph(ABC):
    ...
    def __init__(self, 
        ...
        self._episode_baseline: float = 0.0
    ...
    def run(self, inputs: Dict[str,str], 
        ...
        try:
            if hasattr(self, 'clarify_manager') and self.clarify_manager:
                self.clarify_manager.reset_episode()
    ...
    def set_terminal_reward(self, reward: float, baseline: float = 0.0):
        self._terminal_reward = float(reward)
        self._episode_baseline = float(baseline)
        
        try:
            if hasattr(self, 'clarify_manager') and self.clarify_manager:
                if self.clarify_manager.mode == 'student':
                    self.clarify_manager.e_grpo_update(
                        terminal_reward=self._terminal_reward,
                        baseline=self._episode_baseline
                    )
                    logger.info(f"Terminal E-GRPO update: reward={self._terminal_reward:.4f}, baseline={self._episode_baseline:.4f}")
                else:
                    logger.debug(f"LLM_MODE: Skipping terminal E-GRPO update")
        except Exception as e:
            logger.debug(f"Terminal E-GRPO update failed: {e}", exc_info=True)

    def rl_config(self, **kwargs):
        try:
            if hasattr(self, 'clarify_manager') and self.clarify_manager:
                self.clarify_manager.rl_config(**kwargs)
        except Exception:
            pass
    ...
    def rl_record_interaction(self, *args, **kwargs):
        return 0.0  